...
export function ActionBar() {
  ...
  const canDouble = playerCards.length === 2 && playerStack >= currentHand.betAmount
  ...
  const actions: { action: PlayerAction; label: string; enabled: boolean; cls: string }[] = [
    { action: 'hit',       label: 'Hit',       enabled: true,        cls: 'btn--hit' },
    { action: 'stand',     label: 'Stand',     enabled: true,        cls: 'btn--stand' },
    { action: 'double',    label: 'Double',    enabled: canDouble,   cls: 'btn--double' },
    { action: 'split',     label: 'Split',     enabled: !!canSplit,  cls: 'btn--split' },
    { action: 'surrender', label: 'Surrender', enabled: canSurrender, cls: 'btn--surrender' },
  ]
